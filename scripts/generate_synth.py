"""
Generate a synthetic coastal plume dataset.
Output files under data/: synth_points.csv, grid_coords.csv, flow_meta.json
"""
...
def main(args):
    ...
    ex, ey = flow_basis(args.vx, args.vy)
    ...
    df_grid = pd.DataFrame({"i": gi.ravel(), "j": gj.ravel(), "x": xx.ravel(), "y": yy.ravel()})
    (out_dir/"grid_coords.csv").write_text(df_grid.to_csv(index=False))

    meta = {"vx": float(args.vx), "vy": float(args.vy), "grid": int(L), "noise": float(args.noise), "seed": int(args.seed)}
